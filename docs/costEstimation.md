...
- Document page: about 1,500 input tokens (text or page image) and 800 output tokens ≈ $0.0165 per page on-demand, $0.0083 in batch. 500 pages ≈ $4 in batch.
...
At the assumed usage the platform costs about **$77 per month** to run, or about **$63 per month** during the first year on a new AWS account. Loading the existing 4 TB image collection is a one-time cost of about **$35**. The largest recurring line is the assistant, and the largest fixed line is image storage. Both scale gradually rather than in steps.