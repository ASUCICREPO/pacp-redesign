# Security Check Workflow — Full Reference
...
For each finding: explain the risk, show problematic code with file path and line number, provide concrete fix with code example. Reference the `cic-security` plugin skills (security-iam-secrets, security-data-encryption, security-code-dependencies, security-operations, security-compliance, security-scanning).
